...
test("every AI profile avatar is available and warmed on app startup", async () => {
  assert.deepEqual(
    AI_PROFILE_AVATAR_SOURCES,
  ...
  assert.match(gameSource, /AI_PROFILE_AVATAR_SOURCES\.forEach/);
  assert.match(gameSource, /new Image\(\)/);
