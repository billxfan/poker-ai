...
| Does the shipping archive include development-only code? | SURFACE | The final macOS and Windows `app.asar` archives contain only the three desktop runtime modules plus the production `dist` output and package metadata; test harnesses and `node_modules` are excluded. |
